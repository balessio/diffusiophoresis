function [vp_x,vp_y,vf_x,vf_y,dcdx,dcdy,Vs,dVsdx]=vp_discrete(c,x,y,dx,dy,GpDs,GwDs,L_h)
...
Vs_calc = -GwDs*dc_walldx./c_wall;
dVsdx_calc = -GwDs*(d2c_walldx2./c_wall - (dc_walldx./c_wall).^2);
Vs = zeros(size(xx));
dVsdx = zeros(size(xx));
for i=1:length(y)
    Vs(i,:)=Vs_calc';
